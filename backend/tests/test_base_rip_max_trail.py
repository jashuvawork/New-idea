...
@patch("app.engines.ict_breakout_monitor.get_settings")
@patch("app.engines.dual_mode_strategy.resolve_trading_session_mode", return_value=("DEFENSIVE", {}))
def test_defensive_elite_uses_pad_not_session_day(mock_mode, mock_settings):
    """ELITE pad 28% with session/day 50% must still take (local-base timing)."""
    from app.engines.explosion_detector import ExplosionEvent
    from app.models.schemas import Side
    ...
    event = ExplosionEvent(
        ...
        velocity_3s=3.5, velocity_9s=4.0, velocity_15s=3.0,
        volume_surge=3.0, explosion_score=85.0, tier="ELITE", reason="t",
        daily_move_pct=50.0, peak_move_pct=52.0,
    )
    with patch(
        "app.engines.elite_never_block.top_explosion_must_take_active",
        return_value=False,
    ):
        active, meta = good_day_ict_capture_active(
            state,
            {"NIFTY": snap},
            event=event,
            ict=_ict_flat(session_move_pct=50.0, base_relative_move_pct=28.0),
        )
    assert active is True
    assert meta.get("defensiveBaseRip") is True
    assert meta.get("baseWindowFullLots") is True
