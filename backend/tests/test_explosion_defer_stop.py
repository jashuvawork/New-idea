...
from app.engines.explosion_profit import _defer_adaptive_stop, _effective_stop_points
...
def test_defer_adaptive_stop_aligned_high_chart(mock_conf, mock_exp):
    ...
    trade = PaperTrade(
        ...
        entryContext={
            ...
            "breadth": "BULLISH",
            "selectionScore": 80.0,
            "exitPlan": {"targetPoints": 50.0, "stopPoints": 8.0},
        },
    )
...
def test_no_defer_when_best_already_extended(mock_conf, mock_exp):
    ...
    trade = PaperTrade(
        ...
        entryContext={
            "chartConfidence": 95.0,
            "breadth": "BULLISH",
            "exitPlan": {"targetPoints": 12.0, "stopPoints": 6.0},
        },
    )
...
@patch("app.engines.explosion_profit.get_settings")
@patch("app.engines.confidence_hold.get_settings")
def test_effective_stop_uses_chart_plan_and_confidence_mult(mock_conf, mock_exp):
    s = MagicMock()
    s.chart_confidence_hold_enabled = True
    s.chart_confidence_hold_min_confidence = 62.0
    ...
    trade = PaperTrade(
        ...
        entryContext={
            ...
            "breadth": "BULLISH",
            "selectionScore": 90.0,
            "exitPlan": {"stopPoints": 8.0, "targetPoints": 120.0},
        },
    )
    # 8pt chart SL × 1.4 (95% conf floor) = 11.2pt
    assert _effective_stop_points(trade, 6.0) == 11.2
