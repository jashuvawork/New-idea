...
def get_session_targets() -> OptimizedProfile:
    """Session-adaptive targets — rapid scalp uses tighter micro locks and shorter holds."""
    ...
    if settings.rapid_scalp_mode_enabled:
        if 9 * 60 + 15 <= t < 10 * 60:
            return OptimizedProfile(
                targetPoints=6.0, stopPoints=stop, microTargetPoints=micro,
        ...
        if 11 * 60 + 30 <= t < 13 * 60 + 30:
            return OptimizedProfile(
                targetPoints=4.0, stopPoints=stop, microTargetPoints=micro,
                maxHoldSeconds=120, sessionLabel="midday_rapid",
            )
    ...
    # Standard session targets
    if 9 * 60 + 15 <= t < 10 * 60:
        return OptimizedProfile(
            targetPoints=7.0, stopPoints=stop, microTargetPoints=micro,
...
def check_entry_gate(
    ...
    if (
        settings.rapid_scalp_mode_enabled
        and effective_vel < settings.enhanced_velocity_threshold
    ...
    # Rapid / relaxed scalp gates
    if settings.rapid_scalp_mode_enabled and breadth.bias == "NEUTRAL" and trade_score >= min_score + 3:
        return True, "passed"
...
def evaluate_exit(
    ...
    rapid = settings.rapid_scalp_mode_enabled
    trail_arm = settings.scalp_trail_arm_points if rapid else 2.5
    trail_keep = settings.scalp_trail_keep_ratio if rapid else 0.55
    no_progress_secs = settings.scalp_no_progress_seconds if rapid else 90
    micro_giveback = 0.75 if rapid else 1.25
    ...
    # Micro profit lock — rapid mode banks +1.5pt faster
    if pnl_pts >= profile.microTargetPoints:
        if rapid and hold_seconds >= 8 and (best - pnl_pts >= 0.5 or pnl_pts >= profile.microTargetPoints + 0.5):
            return "simple_micro_profit_lock", pnl_pts * trade.lots * lot_multiplier
        if not rapid and best - pnl_pts >= micro_giveback:
            return "simple_micro_profit_lock", pnl_pts * trade.lots * lot_multiplier
