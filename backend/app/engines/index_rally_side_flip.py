"""Index-rally side-flip bypass — unlock CALL entries after a morning PUT session.

When the index rips off the session low with bullish RSI/MACD (e.g. SENSEX +130pts
76380→76510), stale directional lock + bearish breadth must not block CE entries.
"""
...
def _min_rally_pts(symbol: str, settings: Any) -> float:
    sym = symbol.upper()
    if sym == "NIFTY":
...
def _session_low_and_spot(
    symbol: str,
    snap: SymbolSnapshot,
) -> tuple[float, float]:
    """Session low + live spot from tick tape, broker candles, or snapshot."""
...
def index_rally_metrics(
    ...
    *,
    ...
    """Measure spot rally off session low + chart RSI/MACD for one symbol."""
    settings = settings or get_settings()
    lo, spot = _session_low_and_spot(symbol, snap)
    ...
    return {
        ...
        "minRallyPoints": _min_rally_pts(symbol, settings),
        "rsi": round(rsi, 1),
        "macdBias": macd_bias,
...
def index_rally_side_flip_bypass(
    ...
    *,
    ...
    """
    True when a sustained index rally + bullish RSI/MACD should unlock CALL entries
    despite sticky PUT lock or stale bearish session bias.
    """
    settings = settings or get_settings()
    ...
    require_put_session = bool(
        getattr(settings, "index_rally_side_flip_require_put_session", True)
    )
    if require_put_session:
    ...
    min_pts = float(metrics.get("minRallyPoints") or 0)
    ...
    require_macd = bool(
        getattr(settings, "index_rally_side_flip_require_macd_bullish", True)
    )
